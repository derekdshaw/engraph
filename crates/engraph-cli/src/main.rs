...
fn print_symbol_langs(langs: &[engraph_codegraph::SymbolLangSummary], indent: &str) {
    for l in langs {
...
fn print_repo_plan(repo: &Path, plan: &engraph_codegraph::IndexPlan) {
    println!("DRY RUN — no indexer spawned, no Bazel run, no codegraph writes.");
    println!("repo: {}", repo.display());
    describe_plan(plan, "");
}

fn print_workspace_plan(root: &Path, plans: &[(PathBuf, engraph_codegraph::IndexPlan)]) {
    println!("DRY RUN — no indexer spawned, no Bazel run, no codegraph writes.");
