...
## Codex support (shipped)

engraph now ingests **OpenAI Codex** sessions alongside Claude Code. `ingest_file`
peeks line 1 of a transcript: a Codex rollout leads with `type == "session_meta"`
(from which we hoist session id / cwd / branch, since Codex doesn't repeat those
per message), otherwise it's treated as a Claude transcript. Only the per-line
parse forks (`parse_codex_line` vs `parse_claude_line` → a shared `ParsedLine`);
the offset/rotation/transaction/compress/scope machinery stays shared. Codex lines
carry no per-message uuid, so the `msg_id` is derived deterministically from the
byte offset (`codex-<session>-<offset>`) to keep re-ingest-from-zero idempotent.
We ingest `response_item` messages only (skipping the `event_msg`/`turn_context`
duplicates and `agent_reasoning`); `extract_text` handles `input_text`/`output_text`.
The `session-end` hook ingests any stdin `transcript_path` (format auto-detected)
and now reads `session_id` from the payload, falling back from `CLAUDE_SESSION_ID`.

Remaining follow-ups:

- **Wire the Codex hook (config, not code).** Codex *lifecycle hooks* (not the old
  `notify`, whose payload lacks `transcript_path`/`cwd`) deliver JSON on stdin with
  `session_id`/`cwd`/`transcript_path` — same shape as Claude. A `[[hooks.Stop]]`
  entry running `engraph hook session-end` feeds each turn's rollout incrementally.
  Requires a Codex build with lifecycle hooks (0.38.0 has none).
- **Codex SessionStart brief.** The catch-up ingest runs, but brief *injection* needs
  Codex's `systemMessage` output format, not Claude's
  `hookSpecificOutput.additionalContext`. Add a Codex output path.
- **Multi-day catch-up.** SessionStart catch-up resolves the rollout's parent
  (`~/.codex/sessions/YYYY/MM/DD/`), so it sweeps only that day across all projects
  (each ingested with its own header cwd/scope). Prior days rely on the per-turn
  `Stop` hook; a recursive `~/.codex/sessions` walk filtered by cwd would close it.
