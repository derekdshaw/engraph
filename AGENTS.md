...
A Rust CLI that cuts Codex token usage via persistent session memory,
...
This repo **dogfoods itself** — the `engraph` binary is wired into Codex
hooks (`~/.Codex/settings.json`: pre-bash, pre-grep, post-read, SessionStart,
