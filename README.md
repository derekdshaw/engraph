...
- **Go** — *delegated when configured.* If **`ENGRAPH_BAZEL_SCIP_GO_CMD`** is set, engraph runs it as `<cmd> <repo> <out.scip>` and merges the SCIP it writes (the same contract as Java) — the only way to reach gazelle-managed `go_library` targets that have no `go.mod` (scip-go needs a module root *and* Bazel-resolved deps, both repo-specific). A ready-made best-effort driver ships as **`docs/examples/scip-go-bazel-index.sh`** (scope with `ENGRAPH_BAZEL_SCIP_GO_TARGETS`). Unset → the native multi-module pass: enumerate every `go.mod` under the workspace, run `scip-go` per module, rebase paths to repo-root, and merge — its `symbol[go]: indexed N go.mod modules of M go targets` line makes that pass's coverage gap visible (`go_library` targets without a `go.mod` are unreachable by the native pass).
